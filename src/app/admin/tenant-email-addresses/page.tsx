...
export default function TenantEmailAddressesPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [list, count] = await Promise.all([
        fetchTenantEmailAddressesServer(currentPage, pageSize, tenantId),
        fetchTenantEmailAddressesCountServer(tenantId).catch(() => null),
      ]);
      setItems(list);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await updateTenantEmailAddressServer(selectedItem.id, patch, tenantId);
  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-8" style={{ paddingTop: '180px' }}>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {/* Tenant ID filter – combine with search */}
        <div className="mb-4">
