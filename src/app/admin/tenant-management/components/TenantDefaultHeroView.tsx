...
function displayModeLabel(mode: DefaultHeroDisplayMode | string | undefined): string {
  switch (mode) {
    case 'slideshow':
    ...
    default:
      return mode || 'Slideshow (ordered rotation)';
  }
}
...
export default function TenantDefaultHeroView({
  ...
  const slides = parseTenantDefaultHeroSlides({
    defaultHeroImageUrlsJson: defaultHeroImageUrlsJson ?? undefined,
  });
  const activeCount = slides.filter((s) => s.active).length;
  const maxDisplay = clampHeroMaxDisplayCount(defaultHeroMaxDisplayCount);
  const previewUrls = resolveTenantDefaultHeroUrlsForPreview(slides, maxDisplay);
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
        <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="mt-1 text-sm text-gray-900">{displayModeLabel(defaultHeroDisplayMode ?? 'slideshow')}</dd>
          </div>
          <div>
